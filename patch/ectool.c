...
int cmd_fzpd(int argc, char *argv[])
{
	struct ec_params_fzpd p;
	char *e;
	int cmd;
	int rv;
  ...
  if (cmd == 1 && argc != 4) {
		fprintf(stderr, "Usage: %s %d <seq_id> <msg_idx>\n", argv[0], cmd);
		return -1;
	}

  if (argc == 4) {
	  p.seq_id = strtol(argv[2], &e, 0);
	  p.msg_idx = strtol(argv[3], &e, 0);
		printf("seq_id:%d, msg_idx:%d\n", p.seq_id, p.msg_idx);
  }

	p.cmd = cmd;
	rv = ec_command(EC_CMD_FZPD, 0, &p, sizeof(p), NULL, 0);
	if (rv < 0) {
